...
public class BuyanswerRepository {
    ...
    public Observable<Long> save(ImageCommand imageCommand) {
        return Observable.fromCallable(
                () -> {
                    //TODO: test create buyanswer start -----
                    if (ImageCommand.Create.class.getSimpleName().equals(imageCommand.type)) {
                        Buyanswer buyanswer = Buyanswer.builder()
                                .setUuid(imageCommand.buyanswerUuid)
                                .setVoGeofence(VoGeofence
                                        ...
                                        .build())
                                .setTime2finish(333)
                                .setTitle("测试文章" + imageCommand.buyanswerUuid)
                                .build();
                        roomFascade.daoBuyanswer.save(buyanswer);
                        for (int i = 0; i < 5; i++) {
                            BuyanswerMessage buyanswerMessage = BuyanswerMessage.builder()
                                    .setUuid(UUID.randomUUID().toString())
                                    .setBuyanswerUuid(imageCommand.buyanswerUuid)
                                    .setFloor(i)
                    ...
                    return roomFascade.daoBuyanswer.save(imageCommand);
                }).subscribeOn(Schedulers.io());
    }
    ...
    public LiveData<List<ImageCommand>> listCommandsBy(String commandType, boolean descending, int size) {
        return roomFascade.daoBuyanswer.listCommandsBy(commandType, size);
    }

    public LiveData<List<ImageCommand>> listCommandsBy(String buyanswerId, String commandType, boolean descending, int size) {
        return roomFascade.daoBuyanswer.listCommandsBy(buyanswerId, commandType, size);
    }

    public LiveData<ImageCommand> findLatestCommandBy(String buyanswerId, String comandType) {
